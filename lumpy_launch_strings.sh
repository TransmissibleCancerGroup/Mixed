...
echo "LUMPY_DEPTH_STRING = $LUMPY_DEPTH_STRING"
...
if [[ -z "${SPL_BAM_LIST}${DISC_BAM_LIST}" ]]; then
    echo "# Generate split reads and discordant reads bam files before running this script!"
    exit 1
else
    # parse the libraries in the BAM header to extract readgroups from the same library
    for i in $( seq 0 $(( ${#FULL_BAM_LIST[@]}-1 )) )
    ...
    do
        ...
        | grep -v '^@' \
        | tail -n 1000000 \
        | $PYTHON $PAIREND_DISTRO -r ${LIB_READ_LENGTH_LIST[$j]} -X 4 -N 1000000 -o $FULL_BAM.sample$(($i+1)).lib$(($j+1)).x4.histo \
        > ${TEMP_DIR}/$OUTBASE.sample$(($i+1)).lib$(($j+1)).insert.stats
        # set -o pipefail
    done
    echo "done"
	...
	do
	    ...
	    if [[ "$MEAN" != "NA" ]] && [[ "$STDEV" != "NA" ]]
	    then
		LUMPY_DISC_STRING="$LUMPY_DISC_STRING -pe bam_file:${DISC_BAM},histo_file:$FULL_BAM.sample$(($i+1)).lib$(($j+1)).x4.histo,mean:${MEAN},stdev:${STDEV},read_length:${LIB_READ_LENGTH_LIST[$j]},min_non_overlap:${LIB_READ_LENGTH_LIST[$j]},discordant_z:5,back_distance:10,weight:1,id:${DISC_SAMPLE},min_mapping_threshold:20,${RG_STRING}"
	    fi
	done
    done
fi

echo "# Use this command to run LUMPY... "
echo "
$LUMPY ${PROB_CURVE} \\
    -t ${TEMP_DIR}/${OUTBASE} \\
